...
int3 SectorMap::firstTileToGet(HeroPtr h, crint3 dst)
{
	...
	{
		if(isAccessibleForHero(dst, h)) //pathfinder can find a way using ships and gates if tile is not blocked by objects
			return dst;
...
int3 SectorMap::findFirstVisitableTile(HeroPtr h, crint3 dst)
{
	...
	while(curtile != h->visitablePos())
	{
		...
		if(cb->getPathsInfo(h.get())->getPathInfo(curtile)->reachable())
		{
			return curtile;
		}
		else
